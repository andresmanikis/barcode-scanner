...
function getBinaryCode(segment) {
  const totalWidth = segment.reduce((acc, bar) => acc + bar.width, 0);
  const moduleWidth = totalWidth / 7;

  let binaryCode = "";

  segment.forEach((bar) => {
    const modulesCount = Math.round(bar.width / moduleWidth);
    const digit = bar.color === "WHITE" ? "0" : "1";
    binaryCode += digit.repeat(modulesCount);
  });
...
function processFrame() {
  ...
  const smoothCentralLine = smooth3(centralLine);
  ...
  const binaryCodes = segments.map((s) => getBinaryCode(s));
  console.log(binaryCodes);

  const numbers = binaryCodes.map((bc) => codes[bc]);
  console.log(numbers);
}
